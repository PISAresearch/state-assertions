...
%Table 1 outlines the gas costs for creating the state channel smart contract and its dispute process. 
%As we can see creating the smart contract costs x, and the entire dispute process for a single state assertion costs x. 
%
...
\begin{figure}[H]
	\begin{boxedminipage}{\columnwidth}
		\begin{tabular}{l}
			...
			\quad $\enum$ Status \{$\DEPOSIT$, $\ON$, $\DISPUTE$\} \\
			\quad Status status; $\uintno$ deadline; $\hashblue$ hstate, $\uintno$ i; $\addr$ firstturn; \\ %\comment{// Latest state info}  \\
			\quad $\addr$ asserter; $\hashblue$ prevstate; $\bytes$ input; $\uintno$ cmd;\\
		\end{tabular}
		\\
		\\
		\\
		\begin{tabular}{l}
			\quad $\oninput$ setstate($\bytes[]$ \_sigs, $\uintno$ \_i, $\addr$ \_turn, $\hashblue$ \_newstate) \{ \\
			\quad \quad \require(\_i $>$ i)); \comment{ // Largest counter so far} \\
			\quad \quad $\hashblue$ hmsg = hash(\_i, \_newstate,\_turn, $\addr(\this)$);  \\
			\quad \quad \require(verifySigs(hmsg, sigs, plist));  \comment{// Everyone signed new hstate} \\
			\quad \quad status = Dispute.\ON; refundAllBonds(); \comment{// Cancel dispute and refund bonds} \\
			\quad \quad bestRound = \_i; hstate = \_newstate;  firstturn = \_turn; \comment{// Agreed state} \\
			\quad \}
		\end{tabular}
		\\
		\\
		...
		\begin{tabular}{l}
			...
			\quad \quad $\require$(status == Status.ON \& onlyParties()); \\
			\quad \quad  status = Status.DISPUTE; \\
			\quad \quad deadline = now + disputePeriod; \\
			\quad \}
		\end{tabular}
		\\
		\\
		\\
		\begin{tabular}{l}
			\quad $\oninput$ assertState($\hashblue$ \_prevstate, $\hashblue$ \_newstate, $\bytes$ input, $\uintno$ cmd) \{ \\
			\quad \quad $\require$(status == Status.DISPUTE); \\
			\quad \quad $\require$(\_prevstate == hstate \&\& checkCallerTurn()) \\ 
			\quad \quad asserter = $\msgsender$; \comment{// Store name of asserter} \\ 
			\quad \quad prev\_hstate = \_prevstate; hstate = \_newstate; \\ 
			\quad \quad input = \_input; cmd = \_cmd; \\
		...
		\begin{tabular}{l}
			...
			\quad \quad \require(status == Status.DISPUTE); \\
			\quad \quad \require(asserter != $\msgsender$); \comment{// Stop self-challenging} \\
			\quad \quad \require(prev\_hstate == hash(\_oldstate)); \comment{// Verify old state is correct} \\
			\quad \quad $\hashblue$ temphstate = $\appblue.\transitionblue$(asserter, \_oldstate, input, cmd); \\ 
			\quad \quad $\ifs$(hstate != temphstate) \{ \comment{// Send all coins and bond to non-cheater.}  \} \\
			\quad \}
		\end{tabular}
		\\
		\\
		...
		\begin{tabular}{l}
			...
			\quad \quad \quad $\require$(checkCallerTurn()); \comment{// Let non-asserter challenge or resolve.} \\
			...
			\quad \quad \comment{// Send each party their final balance.} \\
			\quad \}
		\end{tabular}

	   	\begin{tabular}{l}
	   		\quad $\oninput$ deposit(); \comment{// Not implemented due to space} \\
